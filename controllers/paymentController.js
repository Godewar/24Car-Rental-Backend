...
// Mock driver payments data (in-memory storage for demo)
let driverPayments = [
  {
    id: 1,
    driverId: "DR001",
...
/**
 * @desc    Test endpoint to check ZWITCH configuration
 * @route   GET /api/payments/zwitch/test
 * @access  Private
 */
export const testZwitchConfig = (req, res) => {
  res.json({
    configured: !!(ZWITCH_API_KEY && ZWITCH_API_SECRET),
    apiUrl: ZWITCH_API_URL,
    hasApiKey: !!ZWITCH_API_KEY,
    hasApiSecret: !!ZWITCH_API_SECRET,
    apiKeyPrefix: ZWITCH_API_KEY
      ? ZWITCH_API_KEY.substring(0, 15) + "..."
      : "NOT_SET",
...
/**
 * @desc    Process payout via ZWITCH
 * @route   POST /api/payments/zwitch/payout
 * @access  Private
 */
export const processZwitchPayout = async (req, res) => {
  try {
    const {
      driverId,
      amount,
    ...
    if (paymentId) {
      ...
      transaction = await Transaction.findByIdAndUpdate(
        ...
        {
          status:
            zwitchResponse.data.status === "SUCCESS"
              ? "completed"
              : "processing",
      ...
      transaction = await Transaction.create({
        ...
        type: "weekly_payout",
        status:
          zwitchResponse.data.status === "SUCCESS" ? "completed" : "processing",
        transactionId: zwitchResponse.data.id,
        zwitchReferenceId: referenceId,
        method: "bank_transfer",
        accountNumber: `****${accountNumber.slice(-4)}`,
...
/**
 * @desc    Check payout status
 * @route   GET /api/payments/zwitch/status/:referenceId
 * @access  Private
 */
export const getZwitchPayoutStatus = async (req, res) => {
  try {
 ...
 * @route   POST /api/payments/zwitch/verify-account
 * @access  Private
 */
...
/**
 * @desc    Webhook endpoint for ZWITCH callbacks
 * @route   POST /api/payments/zwitch/webhook
 * @access  Public (Webhook)
 */
export const handleZwitchWebhook = async (req, res) => {
  try {
 ...
 * @route   GET /api/payments/drivers
 * @access  Private
 */
export const getAllDriverPayments = (req, res) => {
  res.json(driverPayments);
};

/**
 * @desc    Get single payment by ID
 * @route   GET /api/payments/drivers/:id
 * @access  Private
 */
export const getDriverPaymentById = (req, res) => {
 ...
 * @route   POST /api/payments/drivers/create
 * @access  Private
 */
export const createDriverPayment = (req, res) => {
  try {
    const newId =
      driverPayments.length > 0
        ? Math.max(...driverPayments.map((p) => p.id)) + 1
        : 1;
    const newPayment = {
      id: newId,
      ...req.body,
...
/**
 * @desc    Update payment
 * @route   PUT /api/payments/drivers/:id
 * @access  Private
 */
export const updateDriverPayment = (req, res) => {
  try {
...
/**
 * @desc    Delete payment
 * @route   DELETE /api/payments/drivers/:id
 * @access  Private
 */
export const deleteDriverPayment = (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const index = driverPayments.findIndex((p) => p.id === id);
